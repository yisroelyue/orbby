...
export interface LlmConfig { url: string; apiKey: string; model: string; systemPrompt?: string; usageRules?: string; conversationId?: string }
...
export async function streamComplete(config: LlmConfig, messages: LlmMessage[], tools: unknown[], signal: AbortSignal, onText: (text:string)=>void): Promise<LlmResponse> {
  ...
  const body = {model:config.model,messages,tools:tools.length?tools:undefined,stream:true}; void conversationLog(config.conversationId, 'llm.request', {url:config.url,apiKey:config.apiKey,model:config.model,body});
  ...
  const result = {content,toolCalls:[...calls.values()].map(call=>({id:call.id,name:call.name,arguments:JSON.parse(call.args||'{}')}))}; void conversationLog(config.conversationId, 'llm.response', result); return result;
}
