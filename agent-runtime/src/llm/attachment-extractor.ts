...
function truncate(text: string, limit: number, fileName: string): string {
...
function withTimeout<T>(promise: Promise<T>, fileName: string): Promise<T> {
  return Promise.race([
    ...
    new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error(`「${fileName}」提取超时（${EXTRACT_TIMEOUT_MS}ms）`)), EXTRACT_TIMEOUT_MS)),
  ]);
}
...
function extractPlainText(buffer: Buffer): { text: string; info: string } {
  return { text: decodeTextFile(buffer), info: '文本全文内嵌' };
}

/** PDF 按页提取文本并标注页码（unpdf 惰性加载：纯 JS 封装，无原生编译依赖） */
async function extractPdfText(buffer: Buffer): Promise<{ text: string; info: string }> {
 ...
 */
export async function extractAttachments(attachments: WsAttachment[], signal?: AbortSignal): Promise<void> {
  let budget = MAX_EXTRACT_CHARS_TOTAL;
  for (const attachment of attachments) {
    const kind = attachmentKind(attachment.mimeType);
    if (kind !== 'text' && kind !== 'document') continue;
