...
import { ToolRegistry, ToolContext, ToolResult } from './types.js';

function safePath(input: string, root: string) {
  const path = resolve(root, input);
  if (path.split(sep).includes('.git')) throw new Error('Access to .git is not allowed');
  return path;
}
function params(properties: Record<string, unknown>, required: string[] = []) { return {type:'object', properties, required}; }
function diff(path: string, before: string | null, after: string | null, operation: 'create'|'edit'|'delete'): ToolResult {
  const oldLines = (before ?? '').split(/\r?\n/); const newLines = (after ?? '').split(/\r?\n/);
  let start = 0; while (start < oldLines.length && start < newLines.length && oldLines[start] === newLines[start]) start++;
  let endOld = oldLines.length - 1, endNew = newLines.length - 1; while (endOld >= start && endNew >= start && oldLines[endOld] === newLines[endNew]) { endOld--; endNew--; }
...
export function registerFilesystemTools(registry: ToolRegistry) {
  ...
  registry.register({name:'read', description:'Read a text file with optional line range.', parameters:params({path:text,startLine:{type:'integer'},endLine:{type:'integer'}},['path']), async execute(args, ctx) {
    const path = safePath(String(args.path), ctx.workspacePath); await ctx.permissions?.ensure(path, 'read', ctx.askUser); const value = await fs.readFile(path, 'utf8');
  ...
  registry.register({name:'edit', description:'Replace an exact string in a UTF-8 text file.', parameters:params({path:text,oldString:text,newString:text,replaceAll:{type:'boolean'}},['path','oldString','newString']), async execute(args, ctx) {
    ...
    const after = before.replace(args.replaceAll ? new RegExp(old.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g') : old, String(args.newString)); await fs.writeFile(path, after, 'utf8'); return diff(displayPath(path, ctx.workspacePath), before, after, 'edit');
  }});
  registry.register({name:'delete', description:'Delete a file after permission approval.', parameters:params({path:text},['path']), async execute(args, ctx) {
    const path = safePath(String(args.path), ctx.workspacePath); await ctx.permissions?.ensure(path, 'write', ctx.askUser); const before = await fs.readFile(path, 'utf8'); await fs.unlink(path); return diff(displayPath(path, ctx.workspacePath), before, null, 'delete');
  }});
