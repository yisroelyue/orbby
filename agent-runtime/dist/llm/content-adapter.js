...
export function attachmentKind(mimeType) {
    if (SUPPORTED_IMAGE_MIME.has(mimeType))
        return 'image';
    if (SUPPORTED_TEXT_MIME.has(mimeType))
        return 'text';
    if (SUPPORTED_DOCUMENT_MIME.has(mimeType))
        return 'document';
    return null;
}
...
const MAX_BASE64_LENGTH = { image: 14_000_000, text: 7_000_000, document: 28_000_000 };
/**
...
export function sanitizeAttachments(raw) {
    ...
    for (const item of raw.slice(0, MAX_ATTACHMENTS)) {
        ...
        if (!kind || !data)
            continue;
        if (data.length > MAX_BASE64_LENGTH[kind])
            continue;
...
function documentPart(a) {
    return { type: 'text', text: `[附件: ${a.fileName}｜${a.extractedInfo ?? '已提取文本'}]\n${a.extractedText ?? ''}` };
}
/** 统一用户消息内容：图片块、文档文本块在前，正文（含附件说明）在后 */
export function toUserContent(message, attachments = []) {
    if (!attachments.length)
        return message;
    const parts = attachments
        .filter(a => attachmentKind(a.mimeType) === 'image')
        .map(a => ({ type: 'image', mediaType: a.mimeType, data: a.data }));
    for (const a of attachments) {
        if (attachmentKind(a.mimeType) !== 'image')
            parts.push(documentPart(a));
    }
