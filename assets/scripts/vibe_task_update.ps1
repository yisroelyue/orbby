...
function Remove-OldTaskFiles {
  param([string] $Dir, [int] $MaxAgeHours = 1)

  try {
    $cutoff = (Get-Date).AddHours(-$MaxAgeHours)
    Get-ChildItem -Path $Dir -Filter "vibe_task_*.json" -ErrorAction SilentlyContinue |
...
# Fallback session id so we always have a unique per-session file
if (-not $sessionId) {
  $sessionId = "unknown_" + ([System.Diagnostics.Process]::GetCurrentProcess().Id)
}
...
try {
  ...
  # Per-session file: vibe_task_<sessionId>.json
  $statePath = Join-Path -Path $stateDir -ChildPath "vibe_task_$sessionId.json"
  Write-TextFileWithRetry -Path $statePath -Content $stateJson

  # Clean up stale task files
  Remove-OldTaskFiles -Dir $stateDir

  Write-HookLog "Updated state: event=$hookName status=$status session=$sessionId file=vibe_task_$sessionId.json"
} catch {
