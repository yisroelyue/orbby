...
std::string MultiWindowManager::Create(const flutter::EncodableMap* args) {
  ...
  // CRITICAL: Register DesktopMultiWindowPlugin on the new engine BEFORE the
  // callback. In AOT/release mode, the engine may start executing Dart code
  // as soon as the FlutterViewController is created. If fromCurrentEngine()
  // runs before InternalMultiWindowPluginRegisterWithRegistrar, it throws
  // MissingPluginException because the mixin.one/desktop_multi_window channel
