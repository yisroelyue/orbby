...
class WeatherService {
  ...
  static Future<WeatherData> fetchFromQWeather({
    ...
    log('[Weather] API Host: $apiHost');
    log('[Weather] API Key: $apiKey');

    // 1. 先获取城市 ID（GeoAPI v2 需要认证）
    final geoUrl = Uri.parse(
      'https://$apiHost/geo/v2/city/lookup?location=$city&range=cn',
    );
    log('[Weather] 城市查询: $geoUrl');

    final geoResponse = await http.get(geoUrl, headers: headers).timeout(
      const Duration(seconds: 10),
    );

    log('[Weather] 城市响应 [${geoResponse.statusCode}]: ${geoResponse.body}');
    ...
    log('[Weather] 城市ID: $locationId, 城市名: $locationName');

    // 2. 获取实时天气（使用 Bearer token）
    final weatherUrl = Uri.parse(
      'https://$apiHost/v7/weather/now?location=$locationId',
    );
    log('[Weather] 天气查询: $weatherUrl');

    final weatherResponse = await http.get(weatherUrl, headers: headers).timeout(
      const Duration(seconds: 10),
    );

    log('[Weather] 天气响应 [${weatherResponse.statusCode}]: ${weatherResponse.body}');
    ...
    try {
      ...
      log('[Weather] 指数查询: $indicesUrl');

      final indicesResponse = await http.get(indicesUrl, headers: headers).timeout(
        const Duration(seconds: 10),
      );

      log('[Weather] 指数响应 [${indicesResponse.statusCode}]: ${indicesResponse.body}');

      if (indicesResponse.statusCode == 200) {
        final indicesData = jsonDecode(indicesResponse.body) as Map<String, dynamic>;
    ...
    log('[Weather] 查询成功: ${now['text']}, $temperature');
  ...
  /// 便捷入口
  static Future<WeatherData> fetch({String? city, String? apiKey, String? apiHost}) async {
    log('[Weather] fetch() 开始');
    ...
    log('[Weather] fetch() 参数: apiHost=$actualApiHost, city=$actualCity');

    if (actualApiKey.isEmpty) {
      throw Exception('请先在设置中配置天气 API Key');
    }

    log('[Weather] fetch() 调用 fetchFromQWeather');
